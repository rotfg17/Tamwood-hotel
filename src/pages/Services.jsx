...
const Services = () => {
    ...
    const [newService, setNewService] = useState({ service_name: '', service_description: '', price: '' });
    const [editService, setEditService] = useState({ service_id: null, service_name: '', service_description: '', price: '' });
    ...
    const handleAddService = async () => {
        try {
            const response = await fetch('http://localhost/Tamwood-hotel/api/create-service', {
                method: 'POST',
                headers: {
    ...
    const handleEditService = async () => {
        try {
            const response = await fetch('http://localhost/Tamwood-hotel/api/update-service', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            setEditService({ service_id: null, service_name: '', service_description: '', price: '' });
    ...
    const handleDeleteService = async () => {
        try {
            const response = await fetch('http://localhost/Tamwood-hotel/api/delete-service', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        <div className="services">
            ...
            {editService.service_id && (
                <div className="service-form">
                    ...
                    <button onClick={() => setEditService({ service_id: null, service_name: '', service_description: '', price: '' })}>Cancel</button>
                </div>
            )}
                    ...
                    {services.length > 0 ? (
                        services.map((service) => (
                            <tr key={service.service_id}>
                                ...
                                <td>
                                    <button
                                        onClick={() => setEditService({ service_id: service.service_id, service_name: service.service_name, service_description: service.service_description, price: service.price })}
                                    >
                                        Edit
                                    </button>
